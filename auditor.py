...
BURGER_SINGH_CRITERIA = [
    ...
    {"key": "greeting", "label": "Greeting done properly?", "hint": "check if agent greeted the prospect at start of call. Look for: 'hello', 'hi', 'good morning', 'good afternoon', 'good evening', 'namaste', 'namaskar', any greeting word at beginning of call. Mark true if any greeting was done", "points": 7.5, "group": "Greeting & Introduction", "negative": False},
    {"key": "agent_intro", "label": "Agent introduced themselves by name?", "hint": "check if agent mentioned their own name anywhere in call. Look for: 'mera naam', 'main X baat kar raha', 'I am X', 'this is X', 'myself X', any personal name introduction by the agent. Mark true if agent mentioned their name", "points": 2.5, "group": "Greeting & Introduction", "negative": False},
    {"key": "company_info", "label": "Agent introduced the company (Burger Singh)?", "hint": "check if agent mentioned Burger Singh company name or introduced themselves as calling from Burger Singh. Look for: 'Burger Singh', 'BS', 'company', 'brand', 'hum Burger Singh se', 'calling from Burger Singh'. Mark true if Burger Singh was mentioned as company", "points": 5, "group": "Greeting & Introduction", "negative": False},

    # Group 2 — Ask Right Questions (20 pts)
    {"key": "location_pincode", "label": "Location & Pincode collected?", "hint": "check if location or pincode was asked or mentioned ANYWHERE in call. Look for: 'pin code', 'pincode', 'location', 'city', 'area', 'kahan', 'kahan se', 'kahan rehte', any 6-digit number (pincode), city name mentioned. Mark true if location OR pincode appears anywhere in conversation", "points": 2, "group": "Ask Right Questions", "negative": False},
    ...
    {"key": "prospect_age", "label": "Prospect age collected?", "hint": "check if prospect age was asked or mentioned ANYWHERE in call. Look for: 'age', 'umar', 'kitne saal', 'aap ki age', any number between 18-65 mentioned in context of age, 'young', 'experienced'. Agent may ask directly or indirectly. Mark true if age or any age-related question appears anywhere in conversation", "points": 0.5, "group": "Ask Right Questions", "negative": False},
    ...
    {"key": "reference", "label": "Reference — how did they hear about Burger Singh?", "hint": "check if agent asked how prospect heard about Burger Singh or where they saw the ad. Look for: 'reference', 'kahan se suna', 'kahan dekha', 'ad', 'instagram', 'facebook', 'linkedin', 'google', 'friend ne bataya', 'kaise pata chala', 'source'. Mark true if reference/source was asked anywhere", "points": 2, "group": "Ask Right Questions", "negative": False},
    ...
    {"key": "shop_own_rent", "label": "Shop Own or Rent discussed?", "hint": "check if agent asked whether prospect owns or will rent the shop/space. Look for: 'own', 'rent', 'lease', 'apna', 'kiraye', 'kharida', 'shop', 'space', 'dukan', 'property', 'owned', 'rented', 'agreement', 'jagah'. Mark true if shop ownership or rental was discussed anywhere", "points": 2, "group": "Ask Right Questions", "negative": False},
